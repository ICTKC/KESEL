...
class BertTokenizer(object):
    ...
    def tokenize(self, text, ents):
        split_tokens = []
        split_ents = []
        for token, ent in self.basic_tokenizer.tokenize(text, ents):# 利用BasicTokenizer.tokenize() 基本上处理好了
            mark = True
            for sub_token in self.wordpiece_tokenizer.tokenize(token): # 处理将一个词划分成多个字词情况 puppeteer -> puppet, ##eer
...
class BasicTokenizer(object):
    ...
    def tokenize(self, text, ents):
        ...
        orig_tokens = whitespace_tokenize_ent(text, ents)  # 这个函数起着关键作用
        split_tokens = []
        split_ents = []
