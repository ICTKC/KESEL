...
class BertOnlyELHead(nn.Module):
    def __init__(self, config):
        super(BertOnlyELHead, self).__init__()       
        self.dense1 = nn.Linear(1368, config.hidden_size)  # 3*config.hidden_size  1368  300
        self.activation = nn.ReLU() # torch.nn.GELU() torch.nn.ReLU(),nn.Tanh() 
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.dense2 = nn.Linear(config.hidden_size, 1)
    def forward(self, combine_rep):
        combine_rep = self.dense1(combine_rep)
        combine_rep= self.activation(combine_rep)
        combine_rep= self.dropout(combine_rep)
        entity_linking_score = self.dense2(combine_rep)
        entity_linking_score = torch.squeeze(entity_linking_score, dim=-1)
        return entity_linking_score
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_ent=None, ent_mask=None, output_all_encoded_layers=True):
        ...
        extended_attention_mask = extended_attention_mask.to(dtype=next(self.parameters()).dtype) # fp16 compatibility  将torch.int转为torch.float32
...
class BertForEntityLinking(PreTrainedBertModel):
    def __init__(self, config, num_labels=2):
        super(BertForEntityLinking, self).__init__(config)
        self.bert = BertModel(config)
        self.cls = BertOnlyELHead(config)
        self.apply(self.init_bert_weights)

        self.type_emb = torch.nn.Parameter(torch.randn([4, 5]))  #[4, 8]
        self.linear_embed = torch.nn.Linear(768, 100)
        # self.linear_add = torch.nn.Linear(768, 300)
        self.linear_add = torch.nn.Linear(300, 768)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, \
        input_ent=None, ent_mask=None, entid_embed=None, \
        labels=None, mention_rep=None, entity_rep=None, pem=None, mtype=None, etype=None, local_fea=None):
        n_ments, n_cands = pem.size()
        _, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, input_ent, ent_mask, output_all_encoded_layers=False)
        
        # 方式1：拼接
        # mention_rep = torch.unsqueeze(mention_rep, dim=1)
        # mention_rep = mention_rep.expand(-1,n_cands,-1)
        # pooled_output = torch.unsqueeze(pooled_output, dim=1)
        # pooled_output = pooled_output.expand(-1,n_cands,-1)
        # combine_rep = torch.cat([mention_rep, entity_rep, pooled_output], dim=-1)
        # ernie_scores = self.cls(combine_rep)

        # 方式2： 相加
        # pooled_output = self.linear_add(pooled_output)
        # add_rep = mention_rep + pooled_output
        # add_rep = torch.unsqueeze(add_rep, dim=1)
        # add_rep = add_rep.expand(-1,n_cands,-1)
        # add_rep = add_rep + entity_rep
        # ernie_scores = self.cls(add_rep)
        
        # mention_rep = self.linear_add(mention_rep)
        # entity_rep = self.linear_add(entity_rep)
        # add_rep = mention_rep + pooled_output
        # add_rep = torch.unsqueeze(add_rep, dim=1)
        # add_rep = add_rep.expand(-1,n_cands,-1) 
        # add_rep = add_rep + entity_rep 
        ...
        # 方式3： 相乘
        # mention_rep * entity_rep  
        mention_rep = torch.unsqueeze(mention_rep, dim=1)
        ent_ctx_scores = torch.bmm(entity_rep, mention_rep.permute(0, 2, 1)).view(n_ments, n_cands) 
        # pooled_output * entid_embed
        pooled_output = torch.unsqueeze(pooled_output, dim=1)
        pooled_output = self.linear_embed(pooled_output)
        ernie_scores = torch.bmm(entid_embed, pooled_output.permute(0, 2, 1)).view(n_ments, n_cands) 

        # Typing feature  
        mt_emb = torch.matmul(mtype, self.type_emb)
        et_emb = torch.matmul(etype, self.type_emb)
        tm = torch.sum(mt_emb*et_emb, -1, True)

        inputs = torch.cat([
                            ent_ctx_scores.view(n_ments * n_cands, -1),
                            ernie_scores.view(n_ments * n_cands, -1), 
                            torch.log(pem + 1e-20).view(n_ments * n_cands, -1),
                            tm.view(n_ments * n_cands, -1),
                            local_fea.reshape(n_ments * n_cands, -1)
                            ] , dim=1)
        inputs = inputs.repeat((1, 10))
        scores = self.score_combine(inputs).view(n_ments, n_cands)

        loss_fct = CrossEntropyLoss()
        loss = loss_fct(scores, labels) 
        # loss = F.multi_margin_loss(scores, labels, margin=0.01)
        return loss, scores
