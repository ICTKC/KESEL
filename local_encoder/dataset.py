...
doc2type = pkl.load(open('/home/liuyu/kesel/data/doc2type.pkl', 'rb'))
entity2type = pkl.load(open('/home/liuyu/kesel/data/entity2type.pkl', 'rb'))
mtype2id = {'PER':0, 'ORG':1, 'GPE':2, 'UNK':3}
# Person (PER)、Organization (ORG)、Geographical Entities (GPE)、 Unknow(UNK)  
...
def read_csv_file(path):
    ...
    if path.find('aida')>=0:  # 处理aida_train.csv | aida_testA | aida_testB
        flag = 1
    else:  # 处理 wned-ace2004 | aquaint | ect.
        types = json.load(open('/home/liuyu/kesel/data/generated/type/'+path.split('/')[-1].split('.')[0]+'.json', 'rb')) # 取出文件名xxx
    ...
    with open(path, 'r', encoding='utf8') as f: 
        for i, line in enumerate(f):
            ...
            mention = comps[2]
            mtype = [0,0,0,0]
            if flag == 1:  # 处理aida_train.csv | aida_testA.csv | aida_tesB.csv 
                doc = ''
                for c in doc_name: # 获得每篇doc的name, eg：947testa 947testa
                    try:
                        doc += str(int(c))
                    except:
                        break
                if not doc==docid:
                    docid = doc
                    p = 0
                    tt = doc2type[docid]  # 得到doc中每个mention的类型type，存储形式[['LEIESTERSHIRE','UNK'],['LONDON','GPE],[],...]
                try:

                    while not judge(mention.lower(), tt[p][0].lower()):
                        p += 1
                    mtype[mtype2id[tt[p][1]]] = 1 # 将mtype对应位置置1， mtype=[0,0,0,1] 
                ...
                if path.find('wikipedia')<0:
                    tt = types['sample_%d'%i]['pred'] + types['sample_%d'%i]['overlap'] # types['sample_i]['pred'] 是list类型, ['LOC']
                    for t in tt:
                        if t == 'MISC':
                            t = 'UNK'
                        if t == 'LOC':
                    ...
                    mtype[mtype2id['UNK']] = 1 # 为什么 wned-wikipedia的 type全都置为'UNK'
            lctx = comps[3]
            rctx = comps[4]

            if comps[6] != 'EMPTYCAND':
                cands = [c.split(',') for c in comps[6:-2]]
		# [['61153', '0.76', 'Leicestershire'],['1622318', '0.351', 'Leicestershire County Cricket Club'],...,['23088459,0.003,'Leicestershire County Council election','2009'],...]
                cands = [(','.join(c[2:]).replace('"', '%22').replace(' ', '_'), float(c[1])) for c in cands]
                # [[Leicestershire, '0.76'], ['Leicestershire_County_Cricket_Club', '0.351'], ..., ['Leicestershire County Council election,2009', '0.003']]
            else:
                cands = []
...
def find_coref(ment, mentlist, person_names):
    ...
    coref = []
    for m in mentlist:
        if len(m['candidates']) == 0 or m['candidates'][0][0] not in person_names: # 为啥要在person_names中？
            continue
        ...
        for cur_m in content:
            for i, cand in enumerate(cur_m['candidates']):
                ...
                if cur_m['candidates'][i][0] in entity2type and len(entity2type[cur_m['candidates'][i][0]]) > 0:
                    for t in entity2type[cand[0]]:  # 一个entity可能有多个type
                        cur_m['candidates'][i][-1][mtype2id[t]] = 1
                else:   # 若entity没有类型，则type=[0,0,0,1] 表示 'UNK'
...
#     path = '/home/liuyu/kesel/data/generated/test_train_data/'
